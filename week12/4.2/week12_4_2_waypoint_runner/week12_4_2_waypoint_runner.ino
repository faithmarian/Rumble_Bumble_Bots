...
const int COURSE_PWM = 36;
const int COURSE_PWM_FINAL = 24;
...
const float COURSE_YAW_KP = 0.78f;
const float COURSE_YAW_KD = 0.06f;
const float DRIVE_YAW_DEADBAND = 0.6f;
const float DRIVE_PD_RAMP_PER_SEC = 14.0f;
...
const float STALL_BOOST_MAX_TURN = 42.0f;
const float STALL_BOOST_MAX_DRIVE = 34.0f;
const unsigned long STALL_ABORT_MS = 1800UL;

// ----------------------------- Turns ------------------------
const float TURN_KP = 0.85f;
const float TURN_KD = 0.12f;
const int TURN_PWM_MIN = 20;
const int TURN_PWM_MAX = 30;
const float TURN_PWM_RAMP_PER_SEC = 60.0f;
const float TURN_PWM_BRAKE_PER_SEC = 240.0f;
...
// Closed-loop in-place turn. The minimum PWM is only forced while the output
// pushes toward the target, and a stall boost breaks static friction so a small
// commanded angle can no longer leave the robot buzzing in one place forever.
bool turnTo(float targetYawDeg, const char *status) {
  float error = normalizeAngle(targetYawDeg - yaw());
  ...
  unsigned long limitMs = TURN_TIMEOUT_BASE_MS
                          + (unsigned long)(fabs(error) * TURN_TIMEOUT_PER_DEG_MS);
  unsigned long lastMs = millis();
  unsigned long startMs = lastMs;
  unsigned long stableSince = 0;
  float lastYawValue = yawNow;
  ...
  while (true) {
    ...
    float command = TURN_KP * error - TURN_KD * yawRate;
    float targetOutput = 0.0f;
    if (command * error > 0.0f) {
      float magnitude = constrain(fabs(command),
                                  (float)TURN_PWM_MIN + boost,
                                  (float)TURN_PWM_MAX + boost);
      targetOutput = error > 0.0f ? magnitude : -magnitude;
    }

    if (appliedOutput * targetOutput < 0.0f) appliedOutput = 0.0f;
    // Build up smoothly, but release quickly so damping is not fought by a
    // slowly decaying output that keeps pushing through the target.
    float maxStep = (fabs(targetOutput) < fabs(appliedOutput)
                       ? TURN_PWM_BRAKE_PER_SEC : TURN_PWM_RAMP_PER_SEC) * dt;
...
bool driveRaw(float distanceMm, int direction, float yawTarget, const char *status) {
  ...
  while (travelledMm() < distanceMm) {
    ...
    float targetPwm = (float)DRIVE_PWM_TRIM + boost;
    pwm += constrain(targetPwm - pwm,
                     -DRIVE_PWM_BRAKE_PER_SEC * dt,
                     3.0f * DRIVE_PWM_RAMP_PER_SEC * dt);

    float correction = constrain(COURSE_YAW_KP * error + encoderCorrection(),
                                 -8.0f, 8.0f);
    float forward = direction > 0 ? -pwm : pwm;
    setWheels(forward + direction * correction, forward - direction * correction);

    updateLidars();
    drawTelemetry(status, error, distanceMm - travelledMm());
...
bool driveSegment(float targetMm, bool followWall) {
  ...
  while (true) {
    ...
    float speed = max(0.0f, (alongMm - lastAlongMm) / dt);
    lastAlongMm = alongMm;
    ...
    if (remaining <= DISTANCE_TOLERANCE_MM) break;
    ...
    // ------------------------- speed -------------------------
    int maximumPwm = followWall ? DRIVE_PWM : COURSE_PWM;
    int finalPwm = followWall ? DRIVE_PWM_FINAL : COURSE_PWM_FINAL;
    float distancePd = DRIVE_DISTANCE_KP * remaining - DRIVE_DISTANCE_KD * speed;
    int targetPwm = (int)round(constrain(distancePd,
                                         (float)DRIVE_PWM_TRIM,
                                         (float)maximumPwm));
    if (remaining <= 15.0f || (validRange(frontMm) && frontMm < 55)) {
      targetPwm = DRIVE_PWM_TRIM;
    } else if (remaining <= 60.0f || (validRange(frontMm) && frontMm < 80)) {
      targetPwm = finalPwm;
    }
    ...
    if (!followWall) {
      ...
      if (avoidBias != 0.0f) targetPwm = min(targetPwm, DRIVE_PWM_TRIM);
      if (fabs(lineError) >= COURSE_SLOW_YAW_DEG) targetPwm = DRIVE_PWM_TRIM;
    }

    float boost = stallUpdate(dt, STALL_BOOST_MAX_DRIVE);
    float pwmRamp = (float)targetPwm < pwm ? DRIVE_PWM_BRAKE_PER_SEC
                                           : DRIVE_PWM_RAMP_PER_SEC;
    float maxPwmStep = pwmRamp * dt;
    pwm += constrain((float)targetPwm + boost - pwm, -maxPwmStep, maxPwmStep);
    ...
    float yawRate = normalizeAngle(currentYaw - lastYawValue) / dt;
    lastYawValue = currentYaw;
    float yawKp = followWall ? DRIVE_YAW_KP : COURSE_YAW_KP;
    float yawKd = followWall ? DRIVE_YAW_KD : COURSE_YAW_KD;
    float yawPd = fabs(error) < DRIVE_YAW_DEADBAND
                    ? 0.0f : yawKp * error - yawKd * yawRate;

    int correctionLimit = followWall ? DRIVE_CORRECTION_MAX : COURSE_CORRECTION_MAX;
    float targetCorrection = constrain(yawPd + encoderCorrection(),
