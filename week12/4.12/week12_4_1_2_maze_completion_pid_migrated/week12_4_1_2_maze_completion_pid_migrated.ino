...
// The original week12_4_1_2_maze_completion.ino remains unchanged.

// Week 12 Task 4.1.2: execute the f/l/r path generated by Task 4.1.1.
// Keep the robot still during IMU calibration; it starts automatically after setup.
...
// ------------------------- Control --------------------------
// Marking day: replace only this line with the line printed by the 4.1.1 notebook.
// Current test: image 10.jpg, start (1,7,S), goal (8,2,N).
...
struct Tracker {
  ...
  float update(float lag, float rateSetpoint, float rateActual, float dt) {
    ...
    return constrain(kv * rateSetpoint + kp * lag + ki * integral
                     + kd * (rateSetpoint - rateActual), -outLimit, outLimit);
  }
};
...
bool turnTo(float targetYaw, const char *status) {
  ...
  float total = fabs(delta), setpoint = 0.0f, actual = 0.0f;
  ...
  while (true) {
    ...
    float command = turnTracker.update(setpoint - actual, rate, rateActual, dt)
                    * direction;
...
void driveCell(float &targetYaw, const char *status) {
  ...
  float filteredSpeed = 0.0f, setpoint = 0.0f, lastYaw = yaw();
  ...
  while (true) {
    ...
    lastYaw = currentYaw;
    float forward = driveTracker.update(setpoint - distance, rate, filteredSpeed, dt);
...
bool runCommand(char command, float &targetYaw, uint8_t index, uint8_t count) {
  char status[17];
  snprintf(status, sizeof(status), "RACE %c %u/%u", command, index + 1, count);
...
void setup() {
  ...
  Serial.println(F("Week 12 Task 4.1.2"));
...
void loop() {
  ...
  if (!completed) {
    ...
    for (uint8_t i = 0; i < count; ++i) {
      ...
      if (i + 1 < count) waitStopped(ACTION_PAUSE_MS, "RACE NEXT");
    }
  }

  stopMotors();
  drawTelemetry(success ? "RACE DONE" : "RACE CHECK", 0.0f, 0.0f);
  delay(10);
}
